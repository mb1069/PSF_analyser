...
def get_projections(stack, zstep, zrange=2000):
    ...
    x_profile = mean_p0 / np.max(mean_p0) * 0.5*img.shape[1]
    y_profile = mean_p1 / np.max(mean_p1) * 0.5*img.shape[0]
    ...
    x_profile = mean_p0 / np.max(mean_p0) * 0.5*img_resized.shape[1]
    y_profile = mean_p1 / np.max(mean_p1) * 0.5*img_resized.shape[0]
    
    axs[1].imshow(img_resized, cmap = 'gray')
    axs[1].plot(np.arange(img_resized.shape[1]), x_profile, label = 'x profile')
    axs[1].plot(y_profile, np.arange(img_resized.shape[0]), label = 'y profile')
    axs[1].set_xlabel('Y')
    axs[1].set_ylabel('Z')
    ...
    axs[2].plot(np.arange(img_resized.shape[1]), x_profile, label = 'x profile')
    axs[2].plot(y_profile, np.arange(img_resized.shape[0]), label = 'y profile')
    axs[2].set_xlabel('X')
    axs[2].set_ylabel('Z')
