...
def _gen_all_metrics(stack, px_size, z_step):
    fwhm_xy, fwhm_xy_mse = get_lat_fwhm(stack, px_size)
    fwhm_z, fwhm_z_mse = get_axial_fwhm(stack, z_step)
...
def get_bead_metrics(stacks, locs, px_size, z_step):
    ...
    func(stacks[0])
    with Pool(4) as p:
        r = list(tqdm(p.map(func, stacks.squeeze()), total=n_stacks))
    df = pd.DataFrame.from_records(r)
    locs.reset_index(inplace=True, drop=True)
    locs = pd.concat([locs, df], axis=1)
    return stacks, locs
...
def parse_args():
    ...
    parser.add_argument('-b', '--box-size-length', help='Box size', default=31, type=int)
